...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createBusiness() {
    ...
    const body = { ...req.body };
    if (body?.gps_device?.length > 0) {
      const create_gps_device = body.gps_device;
      body.gps_device = {
        create: create_gps_device,
      };
    } else {
      delete body.gps_device;
    }
    if (body?.server?.length > 0) {
      const create_server = body.server;
      body.server = {
