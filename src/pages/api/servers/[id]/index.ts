...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    ...
    case 'PUT':
      return updateServerById();
    case 'DELETE':
      return deleteServerById();
    default:
      return res.status(405).json({ message: `Method ${req.method} not allowed` });
  }

  async function getServerById() {
    const data = await prisma.server.findFirst(convertQueryToPrismaUtil(req.query, 'server'));
    return res.status(200).json(data);
  }

  async function updateServerById() {
